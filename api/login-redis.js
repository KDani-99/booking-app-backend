...
const {generateAccessToken,generateRefreshToken} = require('../components/tokenmanager-redis');
  ...
  /*  var _captchaValidation = await captchaValidation(req.body.captcha,req.ip)
    ...
    bcrypt.compare(req.body.password,user.password,async (error,result)=>
    {
        ...
        if(result === true)
        {
            ...
            .catch(error=>
            {
                res.status(400).json(RESPONSES.refresh_token_generation_failed).end();
            });
            if(!refresh_token)
                return;
            ...
            req.session.regenerate(function(error)
            {
               ...
               // res.cookie('refresh_token',refresh_token,{httpOnly:true,secure:false,path:'/',domain:'localhost',sameSite:true});
                res.cookie('token',token,{httpOnly:false,secure:false,path:'/',domain:'.localhost.net',sameSite:true});
